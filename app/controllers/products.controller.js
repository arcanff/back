...
export const createProduct = async (req, res) => {
  try {
    ...
    const { name, desc, price, img, size, category } = req.body;

    // Realiza una consulta para insertar el producto en la base de datos
    const result = await pool.query(
      `INSERT INTO products (nombre, descripcion, precio, imagen, talla, categoria) VALUES (?, ?, ?, ?, ?, ?)`,
      [name, desc, price, img, size, category]
    );
...
export const findOneProduct = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `SELECT idproducto, nombre, descripcion, precio, imagen, talla, categoria FROM products WHERE idproducto = ?`,
      [id]
    );
...
export const updateProduct = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE products SET nombre = ?, descripcion = ?, precio = ?, imagen = ?, talla = ?, categoria = ? WHERE idproducto = ?`,
      [name, desc, price, img, size, category, id]
    );
